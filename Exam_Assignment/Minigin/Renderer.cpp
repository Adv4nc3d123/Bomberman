...
void dae::Renderer::RenderTexture(const Texture2D& texture, float x, float y, const SDL_Rect& clip, float angle) const
{
	SDL_Rect dst;
	dst.x = static_cast<int>(x);
	dst.y = static_cast<int>(y);
	SDL_QueryTexture(texture.GetSDLTexture(), nullptr, nullptr, &dst.w, &dst.h);
	SDL_RenderCopyEx(GetSDLRenderer(), texture.GetSDLTexture(), &clip, &dst, angle, nullptr, SDL_FLIP_NONE);
}
...
void dae::Renderer::RenderTexture(const Texture2D& texture, float x, float y, float width, float height,
	const SDL_Rect& clip, float angle) const
{
	SDL_Rect dst;
	dst.x = static_cast<int>(x);
	dst.y = static_cast<int>(y);
